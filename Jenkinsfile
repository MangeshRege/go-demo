pipeline {
  ...
  post {
    ...
    success {
      slackSend(clore:"good", message: "${env.JOB_NAME} completed successfully, deails at ${env.RUN_DISPLAY_URL}")
    }
    failuer {
       slackSend(clore:"danger", message: "${env.JOB_NAME} failed, deails at ${env.RUN_DISPLAY_URL}")
    }
  }
}
