pipeline {
  ...
  post {
    ...
    success {
      slackSend(color:"good", message: "${env.JOB_NAME} completed successfully, deails at ${env.RUN_DISPLAY_URL}")
    }
    failuer {
       slackSend(color:"danger", message: "${env.JOB_NAME} failed, deails at ${env.RUN_DISPLAY_URL}")
    }
  }
}
